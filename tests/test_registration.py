...
from hermes_flaky_detective import IS_FLAKY_SCHEMA, _handle_is_flaky, detect, domain, register, storage
...
def test_register_wires_cli_command():
    ctx = FakeCtx()
    register(ctx)
    assert "flaky-detective" in ctx.cli_commands
    cmd = ctx.cli_commands["flaky-detective"]
